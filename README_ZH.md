...
- ✨ **核心功能**
  ...
  - 标准叠加层 - 保留完整麻将牌与推荐显示，支持自由拖动和缩放
  - 反截图保护 - 可见的主界面与标准 HUD 支持尽力而为的系统级捕获排除
  - HUD 鼠标穿透 - 可点击受保护 HUD 下方的内容，同时保留控制按钮交互
  - 可选托盘图标 - 可隐藏托盘图标，但不会隐藏主界面或任务栏入口
...
Akagi-NG 只保留跨平台的标准 Electron HUD。它沿用现有麻将牌和推荐显示，
并支持自由拖动及缩放。

HUD 右上角始终显示鼠标指针按钮。开启后，HUD 内容区域的点击会穿透到下方游戏
或窗口；将鼠标移动到右上角控制区，仍可关闭穿透或关闭 HUD。鼠标穿透与反截图
保护彼此独立，开关任一功能都不会改变另一功能的状态。

#### 主界面与托盘行为

主界面始终正常启动，并保留 Windows 任务栏入口。启用托盘图标时，关闭主界面会隐藏到托盘；关闭托盘图标时，关闭主界面只会最小化到任务栏，避免程序进入无法恢复的后台状态。

> [!IMPORTANT]
> 反截图不会让主界面从人眼中消失：窗口仍然可见、可交互，只是请求 Windows 将它从受支持的捕获 API 中排除。该保护不能保证覆盖所有 OBS 版本、捕获方式或外部摄像设备。
...
> [!IMPORTANT]
> 内置 mihomo TUN 需要管理员权限。Windows 桌面程序会在启动时请求管理员权限，并由主进程直接管理 mihomo。首次启动 TUN 时请允许 `mihomo.exe` 通过 Windows 防火墙。
>
