...
from akagi_ng.plugins.builtin.majsoul_max.vendor import basic_pb2, liqi_new, liqi_pb2
from akagi_ng.plugins.logger import logger



class MajsoulMaxMod:
    def __init__(self, version, *, settings_path: Path, data_path: Path):
...
''')
        try:
            ...
            logger.warning(
                f'未检测到 mod 配置文件，已生成默认配置：{self.settings_path}')
        self.load_max_data()
        self.SaveSettings()
            ...
            if msg_type == liqi_new.MsgType.Req:
                ...
                match method_name:
                    ...
                    case '.lq.Lobby.setRandomCharacter':
                        ...
                        self.settings['config']['random_character']['pool'] = json_format.MessageToDict(data, always_print_fields_with_no_presence=True, preserving_proto_field_name=True)['pool']
                        self.SaveSettings()
